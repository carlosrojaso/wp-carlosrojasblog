...
class PNFW_Notifications {
 ...
 public function send_chunk($post, $offset) {
  ...
  $subpage_total = 0;
   ...
   pnfw_log($this->type, sprintf(__('Processing new subpage with %d tokens', 'push-notifications-for-wp'), $subpage_total));
   ...
   if ($subpage_total >= 1000) {
    pnfw_log($this->type, sprintf(__('This plugin allows you to send push notifications to up to %d tokens per platform. To send more notifications, please upgrade to <a href="https://www.delitestudio.com/wordpress/push-notifications-for-wordpress/">Push Notifications for Wordpress</a>.', 'push-notifications-for-wp'), 1000));
   }
 ...
 protected function delete_token($token) {
  ...
  if (in_array(PNFW_Push_Notifications_for_WordPress_Lite::USER_ROLE, $user->roles) && empty($user->user_email)) {
   pnfw_log($this->type, sprintf(__("Automatically deleted the anonymous user %s (%s) since left without tokens.", 'push-notifications-for-wp'), $user->user_login, $user_id));
